...
exports.configure = function(c) {
    ...
    YUI.GlobalConfig.combine = true;
    YUI.GlobalConfig.comboBase = yui3.path() + '/build/';
    ...
    */
    YUI.include = function(file, cb) {
        ...
        if (file.match(/^https?:\/\//)) {
            ...
            if (file.indexOf('&') > -1) {
                file = file.replace(YUI.GlobalConfig.comboBase, '');
                ...
                var done = function() {
                    if (files.length === counter) {
                        loaderFn(null, data.join('\n'));
                    }
                }
                files.forEach(function(v, k) {
                    fs.readFile(path.join(YUI.GlobalConfig.comboBase, v), encoding='utf8', (function(v, k) {
                        return function(err, fdata) {
                            counter++;
                            data[k] = fdata;
                            done();
                        }
